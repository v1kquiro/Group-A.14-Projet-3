...
nonce_list = set()            # Liste des nonces déjà vus pour éviter les rejets de connexion               # 0 : bébé calme, 1 : bébé qui pleure
...
# Variables utiles pour les différentes combinaisons du menu principal
            ...
            if debut_appui_A is not None:
                ...
                if duree >= temps_maintenu:
                    ...
                    if A == 1 and B == 1:
                        ...
                        while running_time() - debut_attente < 3000:
                            packet = radio.receive()
                            if packet:
    ...
    if packet:
        type_msg, _, message = receive_packet(packet, key)
        if type_msg == "0x03":
            if message == "temp_basse":
                display.show(Image.SAD)
                music.play(["C4:4"])
            elif message == "temp_haute":
